...
struct model { cfg c; ggml_context*ctx_w=nullptr;
...
static ggml_tensor* weight_f32(ggml_context*c,ggml_tensor*w){return w->type==GGML_TYPE_F16?ggml_cast(c,w,GGML_TYPE_F32):w;}
...
static ggml_tensor* lnorm(ggml_context*c,ggml_tensor*x,ggml_tensor*g,ggml_tensor*b){
  return ggml_add(c,ggml_mul(c,ggml_norm(c,x,LN_EPS),weight_f32(c,g)),weight_f32(c,b));
}
static ggml_tensor* sanm_attn(ggml_context*c,model&m,const std::string&p,ggml_tensor*x,int T){
  ...
  for(int j=0;j<K;j++){auto sl=ggml_view_2d(c,vp,D,T,vp->nb[1],(size_t)j*vp->nb[1]);
    auto wj=weight_f32(c,ggml_view_1d(c,fk,D,(size_t)j*fk->nb[1])); fsmn=ggml_add(c,fsmn,ggml_mul(c,ggml_cont(c,sl),wj));}
...
int main(int argc,char**argv){
  ...
  for(int i=1;i<argc;i++){ if(!strcmp(argv[i],"-m")&&i+1<argc)gguf_path=argv[++i];
    ...
    else {fprintf(stderr,"usage: %s [-m sensevoice.gguf] [--backend cpu|cuda|vulkan] (-a audio.wav | -f fbank.bin) [--vad fsmn-vad.gguf [--vad-maxseg ms]] [--threads N] [--repeat N] [--ids] [--keep-tags]\n",argv[0]);return 1;} }
  if(fbank_path.empty()&&wav_path.empty()){fprintf(stderr,"missing audio or fbank input\n");return 1;}
  if(threads<1||repeat<1){fprintf(stderr,"--threads and --repeat must be positive\n");return 1;}

  if(gguf_path.empty()){fprintf(stderr,"--backend %s requires -m GGUF\n",backend_name.c_str());return 1;}
  ...
  gguf_free(gg);
  const double initialization_ms=(ggml_time_us()-init_start)/1000.0;
  fprintf(stderr,"Backend: %s\nProvider: ggml\nModel: %s\n",backend_name.c_str(),gguf_path.c_str());
  const int F=560,D=m.c.d_model,V=m.c.vocab,P=(int)m.g("ctc.ctc_lo.weight")->ne[1];
  if(P<V){fprintf(stderr,"CTC output size %d is smaller than vocabulary %d\n",P,V);return 1;}
  ...
  std::vector<float>emb_f32((size_t)m.g("embed.weight")->ne[0]*m.g("embed.weight")->ne[1]);
  auto get_weight=[&](ggml_tensor*t,void*dst,size_t size){if(t->buffer)ggml_backend_tensor_get(t,dst,0,size);else memcpy(dst,t->data,size);};
  if(m.g("embed.weight")->type==GGML_TYPE_F32)get_weight(m.g("embed.weight"),emb_f32.data(),ggml_nbytes(m.g("embed.weight")));
  else if(m.g("embed.weight")->type==GGML_TYPE_F16){std::vector<ggml_fp16_t>tmp(emb_f32.size());get_weight(m.g("embed.weight"),tmp.data(),ggml_nbytes(m.g("embed.weight")));ggml_fp16_to_fp32_row(tmp.data(),emb_f32.data(),tmp.size());}
  else{fprintf(stderr,"unsupported embed.weight type %s\n",ggml_type_name(m.g("embed.weight")->type));return 1;}
  const float*emb=emb_f32.data();   // [16, 560] row-major
  // Run encoder+CTC on one fbank window [T,F]; prints greedy-CTC token IDs (no newline).
  double inference_ms=0.0,audio_seconds=0.0;int runs=0;
  auto run_seg=[&](const std::vector<float>& fb,int T){
    ...
    for(int r=0;r<repeat;r++){int64_t compute_start=ggml_time_us();if(ggml_backend_graph_compute(graph_be.backend,gf)!=GGML_STATUS_SUCCESS){fprintf(stderr,"compute failed\n");}
      inference_ms+=(ggml_time_us()-compute_start)/1000.0;runs++;audio_seconds+=(double)T*LFR_N*SHIFT/FS;}
    std::vector<float> lg((size_t)P*N); ggml_backend_tensor_get(logits,lg.data(),0,ggml_nbytes(logits));
    std::vector<int> seg_ids; int prev=-1;   // greedy CTC: argmax per frame -> collapse -> drop blank
    for(int n=0;n<N;n++){ const float*col=&lg[(size_t)n*P]; int am=0; float best=col[0];
  ...
  printf("\n");
  double mean_ms=runs?inference_ms/runs:0.0;
  fprintf(stderr,"[sensevoice] done %.2fs; initialization %.2f ms, inference %.2f ms, RTF %.4f\n",(ggml_time_us()-t0)/1e6,
          initialization_ms,mean_ms,audio_seconds>0.0?inference_ms/(audio_seconds*1000.0):0.0);
