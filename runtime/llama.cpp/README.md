...
|---|---|---|---|
| [Fun-ASR-Nano](fun-asr-nano/) | SenseVoice SAN-M encoder + adaptor + Qwen3-0.6B LLM | `llama-funasr-cli`, `llama-funasr-stream` | validated vs PyTorch |
...
The standalone VAD tool accepts `-t/--threads` to select the CPU inference thread count
and prints its inference time and real-time factor (RTF) after the detected segments:

```bash
llama-funasr-vad -m fsmn-vad.gguf -a audio.wav --threads 4
```

`llama-funasr-vad` also supports `--max-segment-ms`,
`--speech-noise-thres`, `--max-end-silence-ms` (default 800 ms),
`--start-lookback-ms`, `--end-lookahead-ms`,
`--decision-window-ms`, `--speech-start-frames`, `--speech-end-frames`,
and `--merge-gap-ms`. Their defaults preserve the existing FSMN-VAD
behavior; run `llama-funasr-vad --help` for the complete list.

For VAD-only realtime input, build `llama-funasr-vad-stream`. It consumes 16 kHz mono
signed 16-bit little-endian PCM from stdin and emits `speech_start`/`speech_end` events:

The streaming implementation performs true incremental FSMN inference: every FSMN
layer retains only its last `lorder - 1` projected frames, and each call computes the
new frames only. It does not repeatedly run the four-layer network over the previous
76 input frames.

```bash
cat audio.pcm | llama-funasr-vad-stream -m fsmn-vad.gguf --stdin-s16le --threads 4
```

`llama-funasr-stream` uses a dedicated stdin reader and a 60-second PCM ring
buffer, so VAD/ASR compute does not directly stall `arecord`. ASR and VAD thread
counts are independent: `-t`/`--asr-threads` controls the Nano encoder and LLM,
while `--vad-threads` controls FSMN-VAD. The buffer can be changed with
`--audio-buffer-ms`; the final `done` event reports `capture_buffer_peak_ms`.

```bash
arecord -q -D plughw:1,0 -t raw -f S16_LE -c 1 -r 16000 | \
  llama-funasr-stream --enc funasr-encoder-f16.gguf \
  -m qwen3-0.6b-q4km.gguf --vad fsmn-vad.gguf --stdin-s16le \
  --asr-threads 4 --vad-threads 2 --output text
```
