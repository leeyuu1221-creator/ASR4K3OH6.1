...
```
Expected output (one of the benchmark clips; each offline window is printed
with its start and end timestamp):
```
00:00 - 00:15 我想问我在滨海新区有房我一直没有照顾孩子但是我想要抚养权...你觉得这是正常的想法吗
[done] 7.40s ; chunk=15s
```

The CLI remains an offline recognizer: it loads the file, builds its offline
windows, and completes each window synchronously. It does not generate streaming
partial hypotheses. For machine-readable output with window timestamps, use:

```bash
build/bin/llama-funasr-cli \
    --enc funasr-encoder.gguf -m qwen3-0.6b-q8_0.gguf \
    -a audio.wav --segment-ms 15000 --output jsonl
```

批量处理目录或 manifest 可由上层脚本循环调用 `llama-funasr-cli`；设备端只保留
单文件离线识别和流式识别二进制。
